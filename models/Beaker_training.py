...
def valid_gen():
    fasta = pyracular.MatchedKmersGenerator(
        k, "/mnt/ramfs/nt_validation.sfasta", window_size, 8192 * 8
    )
    for i in fasta:
        kmers = list()
...
def gen():
    fasta = pyracular.MatchedKmersGenerator(
        k, "/mnt/ramfs/nt_train.sfasta", window_size, 8192 * 32
    )
    for i in fasta:
        kmers = list()
        kmers.extend([np.concatenate([cls, i["kmers"][0]])])
...
validation_generator = (
    ds.repeat(8192 * 64).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
)
...
metrics = [
    tf.keras.metrics.BinaryCrossentropy(
        name="binary_crossentropy", from_logits=True, label_smoothing=0.3
    ),
    "binary_accuracy",
