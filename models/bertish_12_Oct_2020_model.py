...
cosine_decay_restarts = tf.keras.experimental.CosineDecayRestarts(
    0.005, 8192 * 10, t_mul=2.0, m_mul=1.0, alpha=0.0, name=None
)
...
discriminator_output = discriminator(
    enc_outputs
)  # tf.reshape(enc_outputs, (batch_size, output_dims*window_size)))
...
def valid_gen():
    fasta = pyracular.MaskedKmersGenerator(
        k,
        "/mnt/ramfs/nt_validation.sfasta",
        window_size,
        replacement_pct,
        True,
        8192 * 8,
...
def gen():
    fasta = pyracular.MaskedKmersGenerator(
        k, "/mnt/ramfs/nt_train.sfasta", window_size, replacement_pct, True, 8192 * 32
    )
    for i in fasta:
        yield i["kmers"], i["truth"]
