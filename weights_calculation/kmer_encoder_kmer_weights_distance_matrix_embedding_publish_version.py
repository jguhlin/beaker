...
from tensorflow_model_optimization.sparsity.keras import prune_low_magnitude, PolynomialDecay, UpdatePruningStep, strip_pruning
...
#    magic = prune_low_magnitude(magic, **pruning_params)
    ...
    magic = Dense(dims, activation=tf.nn.swish, name="Magic", use_bias=False, dtype="float32",
#                kernel_regularizer=regularizers.l1(0.05),
		kernel_initializer=tf.keras.initializers.RandomNormal(mean=0., stddev=0.005, seed=1),)
...
np.save("weights/weights_wide_singlelayer_k21_28Jan2021model_" + str(k)
