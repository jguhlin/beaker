...
def model1():
    ...
    reverso = Dense(
        k * ALPHABET_SIZE * dims, use_bias=False, activation=tf.nn.swish, name="Reverso"
    )
    ...
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
        0.1, decay_steps=1, decay_rate=0.99, staircase=False
    )
...
modelckpt = keras.callbacks.ModelCheckpoint(
    filepath="proteindist_{epoch}", save_best_only=True, monitor="val_loss", verbose=1
)
...
np.save(
    "weights/nadam_weights_protein_k" + str(k) + "_dims_" + str(dims),
    np.array([weights]),
)
