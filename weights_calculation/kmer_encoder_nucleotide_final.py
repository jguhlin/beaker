Km  # Set the model parameters here
import os
import aim
from aim import Run
...
cb = AimCallback(repo="./kmer_encoder_log_dir", experiment="relu")
...
def model2(opt):
    ...
    reverso = Dense(
        k * 5 * 3 * dims, use_bias=False, activation=tf.nn.swish, name="Reverso"
    )
...
np.save(
    "weights/weights_wide_singlelayer_k21_17Nov_relu_model_"
    + str(k)
    + "_dims_"
    + str(dims)
    + "_epochs"
