...
num_heads = 8
dropout_rate = 0.15
max_positions = 512 # formerly 512
batch_size = 16
...
def matched_layer():
    return tf.keras.Sequential(
        [
            ...
            tfp.layers.DenseFlipout(1024, activation="relu"),
...
matched_gmp = tf.keras.layers.GlobalMaxPool1D()

out1 = Matched(tf.concat([matched_gmp(enc_outputs_a), matched_gmp(enc_outputs_b)], axis=-1))
out2 = Rc(tf.concat([out1, matched_gmp(enc_outputs_a), matched_gmp(enc_outputs_b)], axis=-1))
...
lr  = tf.keras.optimizers.schedules.CosineDecay(1e-5, 8192 * 16)
...
csd = tf.keras.optimizers.schedules.CosineDecay(5e-5, 4096 * 32, alpha=1e-8)
learning_rate = LinearWarmup(csd, 4096 * 6, 5e-5)
