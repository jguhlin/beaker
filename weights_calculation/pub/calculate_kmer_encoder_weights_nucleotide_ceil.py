...
def model2(opt):
    ...
    output = tf.math.ceil(tf.keras.backend.sum(abs, axis=1))
...
logcb = tf.keras.callbacks.CSVLogger(
    "log_k_{}_dims_{}_activation_{}_ceil.csv".format(k, dims, activation),
    separator=",",
    append=True,
)
...
np.save(
    "weights/wide_singlelayer_k{}_23Apr2023_{}_nucleotide_model_magic_dims_{}_epochs_{}_ceil".format(
        k, activation, dims, epochs
    ),
    magic_weights,
)
...
filename = "weights/wide_singlelayer_k{}_23Apr2023_{}_nucleotide_model_reverso_dims_{}_epochs_{}_ceil".format(
    k, activation, dims, epochs
)
